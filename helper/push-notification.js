...
function sendPushNotification(payload) {
  ...
  androidNotification["sound"] =
    typeof payload.sound == "undefined" ? "default" : payload.sound;
  /*
  ...
  if (typeof payload.notification.title != "undefined") {
    payload.notification.title = entities
      .decode(payload.notification.title)
      .replace(/(<([^>]+)>)/gi, "");
  }

  if (typeof payload.notification.body != "undefined") {
    payload.notification.body = entities
      .decode(payload.notification.body)
      .replace(/(<([^>]+)>)/gi, "");
  }
  ...
  const message = {
    ...
    apns: {
      payload: {
        aps: {
          ...
          sound:
            typeof payload.sound == "undefined"
              ? "default"
              : payload.sound.toLowerCase(),
          alert: {
    ...
    tokens: _.isString(payload.deviceToken)
      ? [payload.deviceToken]
      : payload.deviceToken,
  };
...
function deregisterToken(error, deviceToken) {
  if (FBMessagingErrors.includes(error)) {
    console.log(
      `====== Error push notifications: ${error} with ${deviceToken.length} token failed =======`
    );
  }
}
